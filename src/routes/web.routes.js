...
module.exports = app => {
            ...
            (err, result) => {
                if (result.statusCode == 401) {
                    res.render("login.ejs", {
                        ...
                        info: config.token, client: config.client
                    });
    ...
    app.get("/signup", (req, res) => {
        ...
        console.log(config.client)
        if (config.client == undefined)
            res.render("signup.ejs", { message: "", client: '' });
        else {
                    ...
                    if (access.client._id == clientId) {
                        ...
                        request.get(
                            ...
                            (err, result) => {
                                ...
                                _.forEach(accessId, function(item) {
                                    _.remove(accessId.clients, {_id: clientId});//naso samo da ga uklonim.
    ...
    app.get('/classignments/:id', (req, res) => {
        request.get(config.IdentityRoute + "/api/clients/" + req.params.id, (err, result) => {
